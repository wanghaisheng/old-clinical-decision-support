...
//NOTE: there is no pullIn method for CDSOutput, because it is an output-only structure.  Data is pushed out to it.
    ...
    throws DSSRuntimeExceptionFault, DataFormatException, ImproperUsageException, InvalidDataException {
			...
			throw new ImproperUsageException(errStr);
       		...
       		if (results.get("VMR") != null) {
				...
				List<VMR> sourceVmrList = (List<VMR>)results.get("VMR");
       			...
       			throw new DSSRuntimeExceptionFault("No outputVMR or simpleOutput found by BuildVMRSchemaResultSet.  No result will be returned");
       		}
       	}
