...
public class ObservationResultMapper extends ObservationBaseMapper {
	...
	public static void pullIn(
			org.opencds.vmr.v1_0.schema.ObservationResult 	source, 
			ObservationResult								target, 
		...
		if ( source.getObservationEventTime() != null ) target.setObservationEventTime(MappingUtility.iVLTS2IVLDateInternal(source.getObservationEventTime()));
		if ( source.getObservationValue() != null ) {
			target.setObservationValue(MappingUtility.observationValue2ObservationValueInternal(source.getObservationValue()));
		}
		if ( source.getInterpretation() != null ) {
			target.setInterpretation(new ArrayList<CD>());
			for ( org.opencds.vmr.v1_0.schema.CD oneInterpretation : source.getInterpretation() ) {
				target.getInterpretation().add(MappingUtility.cD2CDInternal(oneInterpretation));
			}
		}

		factLists.internalObservationResultList.add(target);
