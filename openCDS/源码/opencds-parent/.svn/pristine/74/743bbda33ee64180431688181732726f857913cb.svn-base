...
public class DateUtility extends java.lang.Object
{
	// TODO: consider changing methods in this and other utilities that can be turned into static methods into static methods.
	    ...
	    private static DateUtility instance;  //singleton instance

//	    private StringToArrayUtility myStringToArrayUtility;

	    private DateUtility()
	    {
//	        myStringToArrayUtility = StringToArrayUtility.getInstance();
	    }

	    public static synchronized DateUtility getInstance()
	    {
	        if (instance == null)
	        {
	    ...
	    /**
	     * Gets DateWithPrecisionLevel using date in SEBASTIAN string format (yyyy-MM-dd'T'HH:mm:ss or yyyy-MM-dd),
	     * with any precision indicated with an appended text or with ? marks.
	     * <p/>
	     * Returns null if date cannot be created.
	     *
	     * @param dateInSebastianStringFormat
	     * @param formatOfPrecisionLevel      Use static values defined in this class; used to determine if String must be
	     *                                    modified prior to conversion to date.
	     * @return
	     */
/*	    
	    public DateWithPrecisionLevel getDateWithPrecisionLevelFromSebastianString(String dateInSebastianStringFormat, int formatOfPrecisionLevel)
	    {
	        if ((dateInSebastianStringFormat == null) || (dateInSebastianStringFormat.equals("")))
	        {
	            return null;
	        ...
	        if (formatOfPrecisionLevel == DATE_PRECISION_LEVEL_FORMAT_APPENDED_TEXT)
	        {
	            ArrayList dateStringWithoutPrecisionAndPossiblyCarrotPrecision = myStringToArrayUtility.getRegularArrayFromString(dateInSebastianStringFormat, "^");
	            if (dateStringWithoutPrecisionAndPossiblyCarrotPrecision.size() == 0) // should not be the case
	            {
	                return null;
	            }
	            else if (dateStringWithoutPrecisionAndPossiblyCarrotPrecision.size() == 1)
	            {
	                return new DateWithPrecisionLevel(getDateFromSebastianString((String) dateStringWithoutPrecisionAndPossiblyCarrotPrecision.get(0)));
	            }
	            else
	            {
	                return new DateWithPrecisionLevel(getDateFromSebastianString((String) dateStringWithoutPrecisionAndPossiblyCarrotPrecision.get(0)), (String) dateStringWithoutPrecisionAndPossiblyCarrotPrecision.get(1));
	            }
	        }
	        else if (formatOfPrecisionLevel == DATE_PRECISION_LEVEL_FORMAT_QUESTION_MARK)
	        {
	            // assume years with ?? represent 1900, days with ?? represent 1, months with ?? represet 1,
	            // all others with ?? represent 00

	            // yyyy-MM-ddTHH:mm:ss or yyyy-MM-dd
	            // 0123456789012345678    0123456789

	            if ((dateInSebastianStringFormat.length() == 10) || (dateInSebastianStringFormat.length() == 19))
	            {
	                String precisionLevel = null;
	                if (dateInSebastianStringFormat.contains("????"))
	                {
	                    precisionLevel = DateWithPrecisionLevel.PRECISION_UNIT_NONE;
	                }
	                else if (dateInSebastianStringFormat.contains("-??-"))
	                {
	                    precisionLevel = DateWithPrecisionLevel.PRECISION_UNIT_YEAR;
	                }
	                else if (dateInSebastianStringFormat.contains("-??"))
	                {
	                    precisionLevel = DateWithPrecisionLevel.PRECISION_UNIT_MONTH;
	                }
	                else if (dateInSebastianStringFormat.contains("T??:"))
	                {
	                    precisionLevel = DateWithPrecisionLevel.PRECISION_UNIT_DAY;
	                }
	                else if (dateInSebastianStringFormat.contains(":??:"))
	                {
	                    precisionLevel = DateWithPrecisionLevel.PRECISION_UNIT_HOUR;
	                }
	                else if (dateInSebastianStringFormat.contains(":??"))
	                {
	                    precisionLevel = DateWithPrecisionLevel.PRECISION_UNIT_SECOND;
	                }
	                ...
	                if (dateInSebastianStringFormat.length() == 10)
	                {
	                    ...
	                    {
	                        if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_NONE))
	                        {
	                            dateInSebastianStringFormat = "1900-01-01";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_YEAR))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 4) + "-01-01";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_MONTH))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 7) + "-01";
	                        }
	                    }
	                    return new DateWithPrecisionLevel(getDateFromSebastianString(dateInSebastianStringFormat), precisionLevel);
	                }
	                else
	                {
	                    if (precisionLevel != null)
	                    {
	                        if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_NONE))
	                        {
	                            dateInSebastianStringFormat = "1900-01-01T00:00:00";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_YEAR))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 4) + "-01-01T00:00:00";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_MONTH))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 7) + "-01T00:00:00";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_DAY))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 10) + "T00:00:00";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_HOUR))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 13) + ":00:00";
	                        }
	                        else if (precisionLevel.equals(DateWithPrecisionLevel.PRECISION_UNIT_MINUTE))
	                        {
	                            dateInSebastianStringFormat = dateInSebastianStringFormat.substring(0, 16) + ":00";
	                        }
	                    }
	                    return new DateWithPrecisionLevel(getDateFromSebastianString(dateInSebastianStringFormat), precisionLevel);
	                }
	        ...
	        {
	            return new DateWithPrecisionLevel(getDateFromSebastianString(dateInSebastianStringFormat));
	        }
	    }
*/	    

	    /**
	     * Returns DateWithPrecisionLevel in specified format.
	     *
	     * @param dateWithPrecisionLevelAsSebastianString
	     *
	     * @param getInSebastianLongStringFormat If false, returns in Sebastian short string format
	     * @param currentFormatOfPrecisionLevel
	     * @param newFormatOfPrecisionLevel
	     * @return
	     */
	    /*
	    public String getDateWithPrecisionLevelAsSebastianStringInDifferentFormat(String dateWithPrecisionLevelAsSebastianString, boolean getInSebastianLongStringFormat, int currentFormatOfPrecisionLevel, int newFormatOfPrecisionLevel)
	    {
	        DateWithPrecisionLevel dateWithPrecisionLevel = getDateWithPrecisionLevelFromSebastianString(dateWithPrecisionLevelAsSebastianString, currentFormatOfPrecisionLevel);
	        String dateFormat = null;
	        if (getInSebastianLongStringFormat)
	        {
	        ...
	        return getDateWithPrecisionLevelAsString(dateWithPrecisionLevel, dateFormat, newFormatOfPrecisionLevel);
	    }
	    */
	     ...
	     * @return
	     *//*
	    public String getDateAsSebastianLongString (DateWithPrecisionLevel date)
	    {
	        // TODO: callers should instead call DateWithPrecision method directly
	        return date.getSebastianLongString();
	    }*/
	     ...
	     * @return
	     */
	    /*public String getDateAsSebastianShortString (DateWithPrecisionLevel date)
	    {
	        // TODO: callers should instead call DateWithPrecision method directly
	        return date.getSebastianShortString();
	    }*/

	    /**
	     * Returns date in yyyy-MM-dd format, with precision level indicated as explained in
	     * DateUtility.getDateWithPrecisionLevelAsString.
	     *
	     * @param date
	     * @param formatOfPrecisionLevel
	     * @return
	     */
	    /*public String getDateWithPrecisionLevelAsSebastianShortString(DateWithPrecisionLevel date, int formatOfPrecisionLevel)
	    {
	        return getDateWithPrecisionLevelAsString(date, "yyyy-MM-dd", formatOfPrecisionLevel);
	    }*/

	    /**
	     * Returns date in yyyy-MM-ddTHH:mm:ss format, with precision level indicated as explained in
	     * DateUtility.getDateWithPrecisionLevelAsString.
	     *
	     * @param date
	     * @param formatOfPrecisionLevel
	     * @return
	     */
	    /*public String getDateWithPrecisionLevelAsSebastianLongString(DateWithPrecisionLevel date, int formatOfPrecisionLevel)
	    {
	        return getDateWithPrecisionLevelAsString(date, "yyyy-MM-dd'T'HH:mm:ss", formatOfPrecisionLevel);
	    }*/
	    ...
	    /**
	     * Returns date in indicated format.
	     * <p/>
	     * e.g. if formatTemplat == "yyyy-MM-dd":
	     * <p/>
	     * if formatOfPrecisionLevel == DateUtility.DATE_PRECISION_LEVEL_FORMAT_APPENDED_TEXT, if (a)
	     * precisionLevel == null, returns "yyyy-MM-dd", if (b) precisionLevel != null, returns
	     * "yyyy-MM-dd^XXX", where XXX is the precision level defined in 
	     * DateWithPrecisionLevel up to which the date is precise.
	     * <p/>
	     * if formatOfPrecisionLevel == DateUtility.DATE_PRECISION_LEVEL_FORMAT_QUESTION_MARK, if (a)
	     * precisionLevel == null, returns "yyyy-MM-dd", if (b) precisionLevel != null, returns
	     * String with uncertain time units replaced by question marks, e.g. "yyyy-MM-??".
	     * <p/>
	     * if formatOfPrecisionLevel == DateUtility.DATE_PRECISION_LEVEL_FORMAT_IGNORE_PRECISION_LEVEL or
	     * some other input value, returns yyyy-MM-dd without any modification.  Note that this effect
	     * can be achieved by calling the method with the same name where the input parameter is a
	     * java.util.Date rather than a DateWithPrecisionLevel.
	     * <p/>
	     *
	     * @param date
	     * @param formatOfPrecisionLevel
	     * @return
	     */
	    /*public String getDateWithPrecisionLevelAsString(DateWithPrecisionLevel date, String formatTemplate, int formatOfPrecisionLevel)
	    {
	        String timePreciseUpToThisUnit = date.getTimePreciseUpToThisUnit();
	        if (timePreciseUpToThisUnit == null)
	        {
	            return getDateAsString(date, formatTemplate);
	        }
	        else
	        {
	            if (formatOfPrecisionLevel == DATE_PRECISION_LEVEL_FORMAT_QUESTION_MARK)
	            {
	                boolean replaceYearWithQ = false;
	                boolean replaceMonthWithQ = false;
	                boolean replaceDayWithQ = false;
	                boolean replaceHourWithQ = false;
	                boolean replaceMinuteWithQ = false;
	                boolean replaceSecondWithQ = false;
	                boolean replaceMilliSecondWithQ = false;

	                if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_NONE))
	                {
	                    replaceYearWithQ = true;
	                    replaceMonthWithQ = true;
	                    replaceDayWithQ = true;
	                    replaceHourWithQ = true;
	                    replaceMinuteWithQ = true;
	                    replaceSecondWithQ = true;
	                    replaceMilliSecondWithQ = true;
	                }
	                else if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_YEAR))
	                {
	                    replaceMonthWithQ = true;
	                    replaceDayWithQ = true;
	                    replaceHourWithQ = true;
	                ...
	                else if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_MONTH))
	                {
	                    replaceDayWithQ = true;
	                    replaceHourWithQ = true;
	                    replaceMinuteWithQ = true;
	                ...
	                else if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_DAY))
	                {
	                    replaceHourWithQ = true;
	                    replaceMinuteWithQ = true;
	                    replaceSecondWithQ = true;
	                ...
	                else if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_HOUR))
	                {
	                    replaceMinuteWithQ = true;
	                    replaceSecondWithQ = true;
	                    replaceMilliSecondWithQ = true;
	                }
	                else if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_MINUTE))
	                {
	                    replaceSecondWithQ = true;
	                    replaceMilliSecondWithQ = true;
	                }
	                else if (timePreciseUpToThisUnit.equals(DateWithPrecisionLevel.PRECISION_UNIT_SECOND))
	                {
	                    replaceMilliSecondWithQ = true;
	                }
	                ...
	                if (replaceYearWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("y", "?");
	                }
	                if (replaceMonthWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("M", "?");
	                }
	                if (replaceDayWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("d", "?");
	                }
	                if (replaceHourWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("H", "?");
	                }
	                if (replaceMinuteWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("m", "?");
	                }
	                if (replaceSecondWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("s", "?");
	                }
	                if (replaceMilliSecondWithQ)
	                {
	                    formatTemplate = formatTemplate.replace("S", "?");
	                }
	                return getDateAsString(date, formatTemplate);
	            }
	            else if (formatOfPrecisionLevel == DATE_PRECISION_LEVEL_FORMAT_APPENDED_TEXT)
	            {
	                StringBuffer buffer = new StringBuffer();
	                buffer.append(getDateAsString(date, formatTemplate));
	                buffer.append("^");
	                buffer.append(timePreciseUpToThisUnit);
	    ...
	    /**
	     * Returns the count of business days between date1 and date2. Ignores hours, minutes, and seconds.
	     * <code>date1</code> and <code>date2</code> are always treated as business days, even if not
	     * explicitly defined as such.
	     *
	     * @param date1 date to compare with date2
	     * @param date2 date to compare with date1
	     * @return the count of business days difference between date1 and date2
	     */
	    /*public int getBusinessDaysDifference(Date date1, Date date2)
	    {
	        return getBusinessDaysDifference(date1, date2, "");
	    }*/

	    /**
	     * Returns the count of business days between <code>date1</code> and <code>date2</code>. Ignores hours, minutes, and seconds.
	     * <code>date1</code> and <code>date2</code> are always treated as business days, even if not explicitly defined as such.
	     *
	     * @param date1 date to compare against date2
	     * @param date2 date to compare against date1
	     * @param calendarName name of a HolidayCalendar previously registered with LocalDateKitCalculator
	     * @return the count of business days difference between date1 and date2
	     */
	    /*public int getBusinessDaysDifference(Date date1, Date date2, String calendarName)
	    {
	        if (calendarName == null)
	        {
	            calendarName = ""; // Use default holiday calendar; only weekends are non-working days.
	        }
	        // Normalize dates such that date1 is earlier than date2
	        ...
	        LocalDateCalculator dateCalculator = LocalDateKitCalculatorsFactory.getDefaultInstance().getDateCalculator(calendarName, HolidayHandlerType.FORWARD);
	        int diff = 0;

	        for (LocalDate currDate = localDate1; currDate.isBefore(localDate2); currDate = currDate.plusDays(1))
	        {
	            if (!dateCalculator.isNonWorkingDay(currDate) || currDate.equals(localDate1))
	            {
	                ++diff;
	            }
	        }
