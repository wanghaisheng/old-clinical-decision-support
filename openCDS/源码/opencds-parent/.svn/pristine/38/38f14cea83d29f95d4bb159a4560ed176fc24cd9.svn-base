...
//			target.getEvaluatedPersonRelationships().add(e)
   			...
   			throw new DSSRuntimeExceptionFault("No templateId(s) for input EvaluatedPerson found by BuildVMRSchemaResultSet.  No result will be returned.");
		}
			...
			output.setVmrOutput(target);
	   		if (results.get("EvaluatedPerson") != null) {
				...
				List<EvaluatedPerson> sourceEvaluatedPersonList = (List<EvaluatedPerson>)results.get("EvaluatedPerson");
	   			...
	   			throw new DSSRuntimeExceptionFault("No output EvaluatedPerson found by BuildVMRSchemaResultSet.  No result will be returned.");
	   		}		
		}
