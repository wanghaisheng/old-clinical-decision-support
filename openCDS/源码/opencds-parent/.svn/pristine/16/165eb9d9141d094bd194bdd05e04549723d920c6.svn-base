...
//		ANY lANY = new ANY(pANY);
	...
	/**
	 * Builds a String containing either a GUID or an OID plus "^" plus extension
	 * @param context
	 * @param ii
	 * @return
	 */
		...
		try {
			if (  (ii == null ) || ( ii.getRoot() == null ) ) {
				...
				throw new DataFormatException( context + " context: iI2Root(" + ii.toString() 
						+ ") had errors: ID must have GUID as root, or have OID as root plus a non-null extension" );			
			}
			String root = ii.getRoot();
			if ( ( ii.getExtension() != null ) && ( root.contains(".") ) ) {
				// OID, so add on extension
				root = root + "^" + ii.getExtension();
			} else if ( ( ii.getExtension() == null ) && ( !root.contains(".") ) ) {
				//GUID, must not have extension;
				throw new DataFormatException( context + " context: iI2Root(" + ii.toString() 
						+ ") had errors: ID must have GUID as root with no extension, or have OID as root" );			
			}
			return root;
		} catch (Exception e) {
			e.printStackTrace();  
			throw new RuntimeException( "iI2Root(" + ii.toString() + ") had errors: " + e.getMessage() );
		}
	}
	...
	/**
	 * Build an II from a root and optional extension (extension not present when the root is a GUID)
	 * @param rootCaratExtension
	 * @return
	 */
...
//	above changed to below, des 2012-10-05
		...
		if ((pTS == null) || (pTS.getValue() == "") || (pTS.getValue() == "null")) {
			return null;
		}
...
//		lTELExt.setUseablePeriod(sTInternal2ST(pTELInt.getUseablePeriod()));
	...
	public static II getUUIDAsII() {
		...
		II ii = new II();
		ii.setValue( MiscUtility.getUUIDAsString() );
		return ii;
	}
}
