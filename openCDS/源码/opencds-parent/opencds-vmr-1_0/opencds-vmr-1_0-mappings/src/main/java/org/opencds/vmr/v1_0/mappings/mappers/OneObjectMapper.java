...
//					(AdverseEvent)internalVMR, (org.opencds.vmr.v1_0.schema.AdverseEventnew org.opencds.vmr.v1_0.schema, organizedResults);
	 ...
	 * 
	 * @param oneInternalEntityRelationship
	 * @param externalTarget
	 * @param organizedResults
	 * @return
	 * @throws ImproperUsageException
	 * @throws DataFormatException
	...
	public static org.opencds.vmr.v1_0.schema.RelatedEntity pushOutRelatedEntityToClinicalStatement(
			EntityRelationship								oneInternalEntityRelationship, 
			org.opencds.vmr.v1_0.schema.ClinicalStatement	externalTarget,  
			OrganizedResults								organizedResults
		) throws ImproperUsageException, DataFormatException, InvalidDataException {
		
		String _METHODNAME = "pushOutRelatedEntityToClinicalStatement(): "; 
		
		if (oneInternalEntityRelationship == null) {
			String errStr = _METHODNAME + "improper usage: source supplied is null";
			logger.error(errStr);
			throw new ImproperUsageException(errStr);
		}
		...
		Object oneInternalEntityObject = organizedResults.getEntityList().get( ((EntityRelationship)oneInternalEntityRelationship).getTargetEntityId() );
		if (oneInternalEntityObject == null) {
			String errStr = _METHODNAME + "improper usage: looking up entity Object for ID=" + ((EntityRelationship)oneInternalEntityRelationship).getTargetEntityId() + ", and it does not exist.";
			logger.error(errStr);
			throw new ImproperUsageException(errStr);
		}
		
		logger.trace(_METHODNAME + "push out " + oneInternalEntityObject.getClass().getSimpleName() + ", sourceEntity Id " + oneInternalEntityRelationship.getSourceId()
				+ ", targetEntityId " + oneInternalEntityRelationship.getTargetEntityId() 
				+ ", relationshipTimeInterval " + oneInternalEntityRelationship.getRelationshipTimeInterval()
