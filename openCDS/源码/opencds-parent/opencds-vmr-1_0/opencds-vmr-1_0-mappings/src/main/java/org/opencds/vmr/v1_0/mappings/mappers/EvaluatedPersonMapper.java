...
public class EvaluatedPersonMapper extends EntityBaseMapper {
	...
	public static EvaluatedPerson pullIn( 
    	...
    	target.setFocalPerson( subjectPersonId.equals(focalPersonId) );
    	
    	target.setFocalPerson(focalPersonId.equals(subjectPersonId));
    	
		logger.trace(_METHODNAME + "class " + source.getClass().getSimpleName() + ", " + source.getId());
    	if (source.getDemographics() != null) {
    		if (target.getDemographics() == null) {
    			target.setDemographics(new Demographics()); 
    		}
	    	
	    	if (source.getDemographics().getBirthTime() != null) target.getDemographics().setBirthTime(MappingUtility.tS2DateInternal(source.getDemographics().getBirthTime()));
	    	if (source.getDemographics().getGender() != null) {
	    		target.getDemographics().setGender(MappingUtility.cD2CDInternal(source.getDemographics().getGender())); 
	    	} else {
	    		target.getDemographics().setGender(MappingUtility.OPENCDS_NO_INFORMATION); 
	    		}
	    	if ((source.getDemographics().getRace() != null) && (source.getDemographics().getRace().size() > 0))  {
	        	target.getDemographics().setRace(new ArrayList<CD>());
	    		for ( org.opencds.vmr.v1_0.schema.CD oneRaceCD : source.getDemographics().getRace() )  {
	    			target.getDemographics().getRace().add(MappingUtility.cD2CDInternal(oneRaceCD));
	    		} 
	    	}
	    	if ((source.getDemographics().getEthnicity() != null) && (source.getDemographics().getEthnicity().size() > 0)) {
	        	target.getDemographics().setEthnicity(new ArrayList<CD>());
	    		for ( org.opencds.vmr.v1_0.schema.CD oneEthnicityCD : source.getDemographics().getEthnicity() ) {
	    			target.getDemographics().getEthnicity().add(MappingUtility.cD2CDInternal(oneEthnicityCD));
	    		}
	    	}
	    	if (source.getDemographics().getPreferredLanguage() != null) {
	    		target.getDemographics().setPreferredLanguage(MappingUtility.cD2CDInternal(source.getDemographics().getPreferredLanguage()));
	    	}
	    	if (source.getDemographics().getAge() != null) target.getDemographics().setAge(MappingUtility.pQ2PQInternal(source.getDemographics().getAge()));
	    	if (source.getDemographics().getIsDeceased() != null) target.getDemographics().setIsDeceased(MappingUtility.bL2BLInternal(source.getDemographics().getIsDeceased()));
	    	if (source.getDemographics().getAgeAtDeath() != null) target.getDemographics().setAgeAtDeath(MappingUtility.pQ2PQInternal(source.getDemographics().getAgeAtDeath()));
	    	if (source.getDemographics().getName() != null) {
	    		target.getDemographics().setName(new ArrayList<EN>());
	    		for ( org.opencds.vmr.v1_0.schema.EN oneNamePart : source.getDemographics().getName() ) {
	    			target.getDemographics().getName().add(MappingUtility.eN2ENInternal(oneNamePart));
	    		}
	    	}
	    	if (source.getDemographics().getAddress() != null) {
	    		target.getDemographics().setAddress(new ArrayList<AD>());
	    		for ( org.opencds.vmr.v1_0.schema.AD oneAddressPart : source.getDemographics().getAddress() ) {
	    			target.getDemographics().getAddress().add(MappingUtility.aD2ADInternal(oneAddressPart));
	    		}
	    	}
	    	if (source.getDemographics().getTelecom() != null) {
	    		target.getDemographics().setTelecom(new ArrayList<TEL>());
	    		for ( org.opencds.vmr.v1_0.schema.TEL oneTelecomPart : source.getDemographics().getTelecom() ) {
	    			target.getDemographics().getTelecom().add(MappingUtility.tEL2TELInternal(oneTelecomPart));
	    		}
	    	}
    ...
    throws DataFormatException, InvalidDataException, ImproperUsageException, DSSRuntimeExceptionFault 
    {
