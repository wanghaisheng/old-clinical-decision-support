...
import org.opencds.common.interfaces.IBuildResultSet;
 ...
 * 
 */
public class BuildVMRSchemaResultSet implements IBuildResultSet
{
	private static Log logger = LogFactory.getLog(BuildVMRSchemaResultSet.class);
    private static BuildVMRSchemaResultSet instance;  //singleton instance

    private BuildVMRSchemaResultSet()
    {
    }
    ...
    private static final List<String> csNameList = Arrays.asList(clinicalStatementNames);
    ...
    private static final List<String> entityNameList = Arrays.asList(entityNames);
...
//    private List<String> relationshipNameList = Arrays.asList(relationshipNames);
    ...
    public static synchronized BuildVMRSchemaResultSet getInstance()
    //FIXME change ConcurrentHashMaps to WeakHashMap
    {
        if (instance == null)
        {
            instance = new BuildVMRSchemaResultSet();
        }
        return instance;
    }
	...
	public synchronized org.opencds.vmr.v1_0.schema.CDSOutput buildResultSet( 
		...
		DSSRuntimeExceptionFault
	{
		...
		String									focalPersonId		= dssRequestKMItem.getFocalPersonId();
       	...
       	if (results.get("CDSInput") != null) {
       		...
       		try {
       			for (CDSInput internalCdsInput : internalCdsInputList)  
       			{
       				 ...
       				 * 
       				 */
       				CDSOutputMapper.pushOut(results, internalCdsInput, cdsOutput, dssRequestKMItem.getFocalPersonId());
       				...
       				for ( EvaluatedPerson internalEvaluatedPerson : (List<EvaluatedPerson>)results.get("EvaluatedPerson") ) 
       				{
		       									...
		       									for ( ClinicalStatementRelationship oneClinicalStatementRelationship : 
		       										...
		       										if (( oneClinicalStatementRelationship.getTargetId().equals( oneCSResult.getId() ) ) 
		       											...
		       											if ( ((ClinicalStatement)oneResult).isClinicalStatementToBeRoot() == true ) {
		       												String errMsg = "ERROR: BuildVMRSchemaResultSet found a Clinical Statement id "
	       														...
	       														+ ".";
		       												logger.error(errMsg);
		       												//throw new DSSRuntimeExceptionFault(errMsg);
		       											}
	       				...
	       				else 
	       				{
	       					...
	       					for ( int i = 0; i < cdsOutput.getVmrOutput().getOtherEvaluatedPersons().getEvaluatedPerson().size(); i++ ) 
	       					{
	       						...
	       						if ( subjectPersonId.equals(oneOtherEvaluatedPersonOut.getId()) ) 
	       						{
	       							/*
				...
				e.printStackTrace();
				throw new DSSRuntimeExceptionFault("BuildVMRSchemaResult received a DataFormatException while doing a CDSOutputMapper.pushOut: " + e.getMessage());
			} catch (InvalidDataException e) {
				e.printStackTrace();
				throw new DSSRuntimeExceptionFault("BuildVMRSchemaResult received an InvalidDataException while doing a CDSOutputMapper.pushOut: " + e.getMessage());
			} catch (ImproperUsageException e) {
				e.printStackTrace();
				throw new DSSRuntimeExceptionFault("BuildVMRSchemaResult received an ImproperUsageException while doing a CDSOutputMapper.pushOut: " + e.getMessage());
			}
       	}
		return cdsOutput;
	}
