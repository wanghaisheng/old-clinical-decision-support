...
public class EvaluatedPersonRelationshipMapper {
	 ...
	 */
	public EvaluatedPersonRelationship pullIn(org.opencds.vmr.v1_0.schema.EntityRelationship external, MappingUtility mu) {
		
		if (external == null)
			return null;
		
		EvaluatedPersonRelationship target = new EvaluatedPersonRelationship();
		target.setId(MappingUtility.getUUIDAsII().getValue());
		target.setSourceEntityId(MappingUtility.iI2FlatId(external.getSourceId()));
		target.setTargetEntityId(MappingUtility.iI2FlatId(external.getTargetEntityId()));
		target.setTargetRole(MappingUtility.cD2CDInternal(external.getTargetRole()));
		target.setRelationshipTimeInterval(MappingUtility.iVLTS2IVLDateInternal(external.getRelationshipTimeInterval()));
	 ...
	 */
	public org.opencds.vmr.v1_0.schema.EntityRelationship pushOut(EvaluatedPersonRelationship source, MappingUtility mu) {
